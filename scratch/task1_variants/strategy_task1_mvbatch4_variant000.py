# Local sweep variant: batch4 overnight grid charge gentle
...
class Strategy:
    ...
    def grid_charge_target_soc(self, state: dict[str, Any]) -> float:
        ...
        if (0 <= time_of_day <= 24 or 88 <= time_of_day <= 95) and price <= 160.0 and solar <= demand:
            return 0.30
        return 0.0
    ...
    def grid_charge_power_limit(self, state: dict[str, Any], soc: float) -> float:
        ...
        solar = float(state.get("solar", 0.0))
        pre_action_import_mw = max(0.0, demand - solar)
        import_headroom_mw = max(0.0, 90.0 - pre_action_import_mw)
        soc_headroom_mw = max(
            0.0,
            (
        ...
        return min(20.0, import_headroom_mw, soc_headroom_mw)
