# Local sweep variant: combo2 grid assertive + evening hold + prior-day
...
class Strategy:
    ...
    def generate_candidate_actions(self, state: dict[str, Any]) -> list[dict[str, float]]:
        ...
        should_discharge = (
            price >= 430.0
            or self.prior_day_peak_hint(state) >= 430.0
    ...
    def grid_charge_target_soc(self, state: dict[str, Any]) -> float:
        ...
        if (0 <= time_of_day <= 32 or 88 <= time_of_day <= 95) and price <= 210.0 and solar <= demand:
            return 0.60
        return 0.0
    ...
    def grid_charge_power_limit(self, state: dict[str, Any], soc: float) -> float:
        ...
        solar = float(state.get("solar", 0.0))
        pre_action_import_mw = max(0.0, demand - solar)
        import_headroom_mw = max(0.0, 100.0 - pre_action_import_mw)
        soc_headroom_mw = max(
            0.0,
            (
        ...
        return min(30.0, import_headroom_mw, soc_headroom_mw)
    ...
    def desired_soc_floor(self, state: dict[str, Any]) -> float:
        ...
        if 55 <= time_of_day <= 70 and self.prior_day_future_peak_hint(state) >= 430.0:
            return 0.18
