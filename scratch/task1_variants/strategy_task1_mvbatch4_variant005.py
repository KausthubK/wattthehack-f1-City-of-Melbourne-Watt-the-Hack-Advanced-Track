# Local sweep variant: batch4 cheap-only high target
...
class Strategy:
    ...
    def grid_charge_target_soc(self, state: dict[str, Any]) -> float:
        ...
        if (0 <= time_of_day <= 28 or 88 <= time_of_day <= 95) and price <= 150.0 and solar <= demand:
            return 0.70
        return 0.0
    ...
    def grid_charge_power_limit(self, state: dict[str, Any], soc: float) -> float:
        ...
        return min(35.0, import_headroom_mw, soc_headroom_mw)
