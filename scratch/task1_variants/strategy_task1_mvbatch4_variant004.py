# Local sweep variant: batch4 low demand cap grid charge
...
class Strategy:
    ...
    def grid_charge_target_soc(self, state: dict[str, Any]) -> float:
        ...
        if (0 <= time_of_day <= 36 or 88 <= time_of_day <= 95) and price <= 200.0 and solar <= demand:
            return 0.55
        return 0.0
    ...
    def grid_charge_power_limit(self, state: dict[str, Any], soc: float) -> float:
        ...
        solar = float(state.get("solar", 0.0))
        pre_action_import_mw = max(0.0, demand - solar)
        import_headroom_mw = max(0.0, 85.0 - pre_action_import_mw)
        soc_headroom_mw = max(
            0.0,
            (
